...
def evaluate(full_con, const_con, gamma_sin, eta_sin, lambda2):
    val = 1.0
    val_expr = 0.0
    print full_con
	...
	for j in range(len(full_con[i])):
		...
		if up[0].name != lo[0].name or up[0].kind != lo[0].kind :
		    val = 0.0
		#print 'delta va; - ', full_con[i][j].upper, full_con[i][j].lower
		...
		print 'gamma va; - ', gamma_sin[int(up[0].name)][int(lo[0].name)], full_con[i][j].upper, full_con[i][j].lower
